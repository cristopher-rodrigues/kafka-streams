...
class CaseClassSerde[CC](schemaRegistryUrl: String = "http://kafka-schema-registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Serde[CC] {
  private class CaseClassDeserializer(schemaRegistryUrl: String = "http://kafka-schema-registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Deserializer[CC] {
  ...
  private class CaseClassSerializer(schemaRegistryUrl: String = "http://kafka-schema-registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Serializer[CC] {
